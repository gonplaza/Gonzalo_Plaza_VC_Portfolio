...
class Startup(Agent):
    ...
    def time_progression(self):
        # if no VC has invested in the startup yet, then progress without considering advisory from VC
        if len(self.VC_investments) == 0:
    ...
    def noise_after_DD(self):
        ...
        # if no VC investors yet, growth_with_noise is used as an approximation to VC Quality as the matching is done between these two - /
...
class World(Model):
    ...
    def step(self):
        ...
        print("Startups:", len(self.schedule_1.agents))
