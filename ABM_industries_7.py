...
for sim in range(number_of_simulations):
    ...
    Industry_loc = {"IT": 0.475, "B2C": 0.500, "Materials": 0.539, "B2B": 0.577, "Financials": 0.585, "Energy": 0.593, "Health": 0.616}
    Industry_scale = {"IT": 0.466, "B2C": 0.49, "Materials": 0.529, "B2B": 0.566, "Financials": 0.574, "Energy": 0.582, "Health": 0.605}
    ...
    # Startup Coefficeints - Industries - same probability for each - random choice
    List_of_Industries = ["IT", "B2C", "Materials", "B2B", "Financials", "Energy", "Health"]
    Probability_Distribution_of_Industries = [1/7, 1/7, 1/7, 1/7, 1/7, 1/7, 1/7]
    # Matrix of correlations between industries
    Industry_correlation_matrix = pd.read_excel("Industry7_Correlation_Matrix.xlsx")
    Industry_correlation_matrix.index = Industry_correlation_matrix["Unnamed: 0"]
    del Industry_correlation_matrix["Unnamed: 0"]
    ...
    Noise_sd_before_DD = {"IT": 0.307, "B2C": 0.322, "Materials": 0.347, "B2B": 0.372, "Financials": 0.377,"Energy": 0.382, "Health": 0.397} 
    ...
    class VC(Agent):
        ...
        def expected_portfolio_variance(self, Portfolio):
            ...
            for i in Portfolio:
                    for j in Portfolio:
                        ...
                        sd_i = (Noise_sd_before_DD[getattr(i[0], "Industry")]**2 + Idiosyncratic_risk_sd**2)**(1/2)
                        sd_j = (Noise_sd_before_DD[getattr(j[0], "Industry")]**2 + Idiosyncratic_risk_sd**2)**(1/2)
                        Correlation_coeff =  float(Industry_correlation_matrix.loc[getattr(i[0], "Industry"), getattr(j[0], "Industry")]) 
        ...
        def get_state(self, Prospect): 
            ...
            for i in self.Portfolio:    
                Avg_corr =  float(Industry_correlation_matrix.loc[getattr(Prospect, "Industry"), getattr(i[0], "Industry")]) + Avg_corr
    ...
    class Startup(Agent):
        ...
        def noise_before_DD(self):
            ...
            if self.Life_stage == 0:
                noise = np.random.normal(Noise_mean_before_DD, 2*Noise_sd_before_DD[self.Industry])
            ...
            # If the startup is not new, the noise is reduced in proportion to the maturity of the startup
                ...
                noise = np.random.normal(Noise_mean_before_DD, 2*Noise_sd_before_DD[self.Industry]/(self.Life_stage**(1/2)))
                self.Growth_with_noise = self.Growth + noise
                # Keep Growth with noise within limits
                while self.Growth_with_noise < -1:
                    self.Growth_with_noise = self.Growth_with_noise + abs(noise)
        ...
        def noise_after_DD(self):
            ...
            if self.Life_stage == 0:
                noise = np.random.normal(Noise_mean_before_DD, 2*Noise_sd_before_DD[self.Industry]*(1.5-noise_reduction_VC_quality))
                ...
                noise = np.random.normal(Noise_mean_before_DD, 2*Noise_sd_before_DD[self.Industry]*(1.5-noise_reduction_VC_quality)/(self.Life_stage**(1/2)))
                self.Growth_after_DD = self.Growth + noise
                # Keep perceived Growth within limits
                while self.Growth_after_DD < -1:
                    self.Growth_after_DD = self.Growth_after_DD + abs(noise)
    ...
    class World(Model):
        ...
        def generate_startups(self):        
            for j in range (self.schedule_1.steps*Number_of_new_startups, (self.schedule_1.steps+1)*Number_of_new_startups):
                # Get an industry by random choice 
                Industry = random.choices(List_of_Industries, Probability_Distribution_of_Industries)[0]
                # create the startup, and assign a revenue growth based on the distribution corresponding to the industry
                b = Startup(j, skewnorm.rvs(Growth_a, Industry_loc[Industry], Industry_scale[Industry]), Industry, 0, self)
...
df2 = pd.DataFrame(np.array(Portfolio_data), columns = ["Simulation_Number","Unique_id_VC", "Unique_id_Startup", "Growth_final", "Return","Amount_Invested", "Growth_after_DD", "Fund_age_when_invested", "Industry"])
